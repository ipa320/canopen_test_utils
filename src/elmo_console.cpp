...
class ElmoNode: public Node
{
    static ObjectDictSharedPtr  make_dict(){
        ...
        dict->insert(false, ROSCANOPEN_MAKE_SHARED<const canopen::ObjectDict::Entry>(ObjectDict::VAR, 0x1024, ObjectDict::DEFTYPE_UNSIGNED8, "OS command mode", false, true, false));

        dict->insert(false, ROSCANOPEN_MAKE_SHARED<const canopen::ObjectDict::Entry>(ObjectDict::VAR, 0x1001, ObjectDict::DEFTYPE_UNSIGNED8, "error register", true, false, false));

        dict->insert(false, ROSCANOPEN_MAKE_SHARED<const canopen::ObjectDict::Entry>(ObjectDict::VAR, 0x1017, ObjectDict::DEFTYPE_UNSIGNED16, "producer heartbeat", true, true, false, HoldAny((uint16_t)0)));
...
public:
    ...
    ElmoNode(can::CommInterfaceSharedPtr interface, const uint8_t &id) : Node (interface, make_dict(), id){
...
void shutdown(ThreadedSocketCANInterfaceSharedPtr &driver, boost::shared_ptr<ElmoNode> &node, int code){
    state_printer.reset();
